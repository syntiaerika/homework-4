...
app.post('/shopping-list/create', [
  body('name').isString().withMessage('Názov zoznamu musí byť string.'),
  body('members').isArray().optional().withMessage('Členovia musia byť vyplnený.'),
], authorize('Owner'), async (req, res) => {
  ...
  try {
    const newList = new ShoppingList({ name, owner: req.headers['user-id'], members });
    await newList.save();
...
app.patch('/shopping-list/:id/item/:itemId/complete', [
  ...
  body('checked').isBoolean().withMessage('Hodnota "checked" musí byť boolean.'),
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
...
app.delete('/shopping-list/:id/delete', [
  ...
  try {
    ...
    return res.status(200).json({ message: 'Nákupný zoznam bol zmazaný.' });
  } catch (err) {
    console.error(err); 
    return res.status(500).json({ error: 'Chyba pri mazaní zoznamu' });
  }
});
...
app.get('/shopping-list/list', async (req, res) => {
  try {
    ...
    console.error(err); 
    return res.status(500).json({ error: 'Chyba pri načítaní zoznamov' });
  }
});


app.get('/shopping-list/get/:id', [
  param('id').isMongoId().withMessage('Neplatné ID zoznamu.'),
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
  ...
  try {
    ...
    console.error(err); 
    return res.status(500).json({ error: 'Chyba pri načítaní zoznamu' });
  }
});
